...
class RouteLearner {
  ...
  Future<LearnResult> processCompletedTrip(Trip trip) async {
    ...
    if (match != null) {
      ...
      final tripDurationMin = trip.endTime
      ...
      await _db.updateCandidate(
        RouteCandidatesCompanion(
          id: Value(match.id),
    ...
    final dayOfWeek = trip.startTime.weekday;
    final tripDurationMin = trip.endTime?.difference(trip.startTime).inMinutes;
    ...
    await _db.insertCandidate(
      RouteCandidatesCompanion.insert(
        id: const Uuid().v4(),
